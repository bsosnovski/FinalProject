...
If the TF-IDF is closer to zero, it indicates that the word is extremely common. Thus, the words "staten", "island" and "ny" are not so important for the whole corpus. On the other hand, the words with high TD-IDF are important. As one can see, they have high TD-IDF due to typos and concatenation of words together. Nevertheless, note that majority of them are included some of the most frequent positive terms of the sentiment analysis.

# Conclusions

The project's goal is to find out if people's feeling and/or opinions is  positive or negative. 
...
The LDA analysis also seems to confirm this since the topics presented by the model present top frequent positive words as "congratulations" and "proud". But none of the top negative words appear among the topics from the tweets involvng the terms related to the election that were in the search.

Finally the TF-IDF analysis shows that the important word, even though some are formed by concatenations, mostly present positive terms and scarcely negative term(s) as in "fake". 

Overall the sentiment present in the tweets are in positive terms.
